...
"""
   ...
   # 5. 도구 목록 준비 (convert_currency 추가: 외부 환률 반영)
    ...
    # 참고: 이전 코드의 create_openai_tools_agent는 범용적인 함수입니다.
    # 만약 대화 기억 기능이 잘 안된다면, memory를 지원하는 다른 에이전트(예: ConversationalAgent)로 변경이 필요할 수 있습니다.
    # 하지만 우선 현재 구조를 유지하며 프롬프트와 도구만 수정합니다.
